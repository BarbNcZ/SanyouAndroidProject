...
fun TaskTypesDropDownMenu(
    ...
    ExposedDropdownMenuBox(
        ...
        ExposedDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
            options.forEach { taskType: GetTipoTarefaResult ->
                DropdownMenuItem(
                    text = { Text(text = taskType.dsTipoTarefa) },
                    onClick = {
                        expanded = false
                        selected = taskType.dsTipoTarefa
                        onValueChangedEvent(taskType.cdTipoTarefa)
                    }
                )
            }
        }
